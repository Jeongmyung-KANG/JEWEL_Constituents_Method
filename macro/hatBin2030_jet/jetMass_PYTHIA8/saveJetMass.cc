...
int main(int argc,char** argv)
{
  ...
  TH1F* jetMass = new TH1F("jetMass","jetMass",400,0.0,200.0);
    ...
    for(Int_t i=0; i< jets.size(); i++){
        if(TMath::Abs(jets[i].eta()) < R + 2.4){
           ...
           if(40 < jets[i].pt() && jets[i].pt() < 60){
                jet40_60++;
		jetMass->Fill(jets[i].m());
                //jet size normalization
                //jetMass->SetBinContent(i,jetMass->GetBinContent(i)/jets.size());
           }
        }
    }
  ...
  jetMass->Scale(1.0/jetn);
  jetMass->Scale(1.0/jetMass->GetXaxis()->GetBinWidth(1));
...
//g++ -g saveJetMass.cc -o saveJetMass.exe `root-config --cflags` `root-config --evelibs` `../fastjet-3.4.2/fastjet-config --cxxflags --libs`
