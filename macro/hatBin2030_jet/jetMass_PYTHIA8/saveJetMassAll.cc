...
int main(int argc,char** argv)
{
  ...
  for(Int_t iev = 0; iev < nentries; iev++)
  {
    ...
    particles->Clear();
    tin->GetEntry(iev); //마법의 코드 , particles에도 정보가 알아 서 들어가짐.
    ...
    //making jet
    Double_t R = 0.4;
    JetDefinition jet_def(antikt_algorithm, R); //jet란 이런 것이 다!
...
//g++ -g saveJetMassAll.cc -o saveJetMassAll.exe `root-config --cflags` `root-config --evelibs` `../fastjet-3.4.2/fastjet-config --cxxflags --libs`
