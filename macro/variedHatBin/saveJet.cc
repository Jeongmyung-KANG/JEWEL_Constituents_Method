...
int main()
{
  ...
  for(Int_t iev = 0; iev < nentries; iev++)
  {
    ...
    for(Int_t i=0; i<jets.size(); i++){
      //cout << " PT입니다 . "<<jets[i].pt() <<endl;
      if(TMath::Abs(jets[i].eta()) < R + 2.4){
        //cout<<iev<<" "<<i<<" "<<jets[i].pt()<<endl;
  ...
  TFile* fout = TFile::Open("JetPt510.root", "RECREATE");
...
//g++ saveJet.cc -o saveJet.exe `root-config --cflags` `root-config --evelibs` `../fastjet-3.4.2/fastjet-config --cxxflags --libs`